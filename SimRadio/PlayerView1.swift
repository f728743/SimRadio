...
//  SimRadio
//
//  Created by 18720677 on 08.01.2021.
//
...
struct PlayerView1: View {
    ...
    let mediaSource = MediaSource(
        ...
        coverArt: UIImage(named: "radio_01_class_rock")!
    )
...
//        .padding(.bottom, safeArea?.bottom == 0 ? 15 : safeArea?.bottom)
    ...
    var miniPlayer: some View {
        ZStack {
            ...
            //                }
            HStack {
                Text(mediaSource.title).padding(.leading, Constants.maximizedTitlePadding)
                Spacer(minLength: 0)
                HStack(spacing: 20) {
    ...
    var coverArt: some View {
            HStack {
                ...
                ZStack {
                    ...
                    Image(uiImage: mediaSource.coverArt)
                        .resizable()
                        .frame(width: coverArtSize, height: coverArtSize)
...
private extension PlayerView1 {
    ...
    enum Constants {
        static func cornerRadius(for shape: PlayerShape) -> CGFloat { shape == .maximized ? 40 : 0 }
        static func bottomInset(for shape: PlayerShape) -> CGFloat { shape == .maximized ? 0 : minimizedBottomInset }
